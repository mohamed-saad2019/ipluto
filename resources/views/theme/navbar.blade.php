                        ...
                        <li class="nav-item dropdown">
                            ...
                            <div class="dropdown-menu">
                                ...
                                @if(\App\ChildCategory::where('status', '1')->GroupBy('slug')->orderBy('id','ASC')->get())
                                    @foreach(\App\ChildCategory::where('status', '1')->GroupBy('slug')->orderBy('id','ASC')->get() as $child)
                                    <div class="dropdown-item-list">
                                        @if(count(\App\SubCategory::whereIN('id' , \App\ChildCategory::where('slug' , $child->slug)->pluck('subcategory_id'))->get()) > 0 )
                                        <ul class="dropdown-item-ul">
                                        @foreach(\App\SubCategory::whereIN('id' , \App\ChildCategory::where('slug' , $child->slug)->pluck('subcategory_id'))->get() as $grade) 
                                            <li> <a href="{{route('teachers',[$child->id,$grade->id])}}">{{$grade->title}}</a> </li>
                                        @endforeach
                                        </ul>
                                        @endif
                        ...
                        @auth
                            <div class="iconUserShp" style="display: contents;">
                                ...
                                @elseif(Auth::User()['role'] == 'user')
                                    <a href="{{route('student.profile')}}"><button class="form-control" style="margin-top: 9px">
                                        My Dashboard
                                    </button></a>
                                @endif
                                <span><i class="fas fa-shopping-cart"></i></span>
                                @if(Auth::User()['user_img'] != null && Auth::User()['user_img'] !='' && @file_get_contents('images/avatar/'.Auth::user()['user_img']))
                                    
                                    <span><img src="{{ url('/images/avatar/'.Auth::User()->user_img) }}" style="width: 35px;height: 35px;border-radius: 50%;" class="dropdown-user-circle" alt=""></span>
                                @else
