...
@if($invoice['border_enable'] == '1')
  .border-one { 
    ...
    border: 15px {{ $invoice['border_style'] }} {{ $invoice['border_color'] }}; 
    padding:20px;
          ...
          <div class="col-xs-12">
            <div class="page-header">
              ...
              @php
                ...
                @endphp
              <small class="purchase-date">{{ __('frontstaticword.Puchasedon') }}: {{ date($test, strtotime($orders['created_at'])) }}</small>
            </div>
          </div>
          <!-- /.col -->
        </div>
              ...
              <tr class="view-order">
                ...
                <td class="txt-rgt">
                  @if($orders->coupon_discount == !NULL)
                    @if($contains) <i class="fa {{ $orders['currency_icon'] }}"></i> @else {{ $orders['currency_icon'] }} @endif{{ $orders['total_amount'] - $orders['coupon_discount'] }}
                  @else
                     @if($contains) <i class="fa {{ $orders['currency_icon'] }}"></i> @else {{ $orders['currency_icon'] }} @endif{{ $orders['total_amount'] }}
                  @endif
                </td>
