...
<div class="col-md-4 col-lg-4">
  <div class="widgetbar">
    <button type="button" class="float-right btn btn-danger-rgba mr-2 " data-toggle="modal"
      data-target="#bulk_delete"><i class="feather icon-trash mr-2"></i> Delete Selected</button>
    <a href="{{url('bundle/create')}}" class="float-right btn btn-primary-rgba mr-2"><i
        class="feather icon-plus mr-2"></i>Add Bundle</a>
    ...
    <div class="col-lg-12">
      <div class="card m-b-30">
        <div class="card-header">
          <h5 class="card-box">All Bundles</h5>
        </div>
        <div class="card-body">
          ...
          <div class="table-responsive">
            ...
            <div id="bulk_delete" class="delete-modal modal fade" role="dialog">
                <div class="modal-dialog modal-sm">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <form id="bulk_delete_form" method="post"
                                action="{{ route('bundlecourse.bulk_delete') }}">
                                @csrf
                                @method('POST')
                  ...
                  <td>
                    <label class="switch">
                      <input class="featuredstatus" type="checkbox"  data-id="{{$cat->id}}" name="status" {{ $cat->featured == '1' ? 'checked' : '' }}>
                      <span class="knob"></span>
                    </label>
                    {{-- <input  data-id="{{$cat->id}}" type="checkbox"  class="custom_toggle" name="status" {{ $cat->status == '1' ? 'checked' : '' }} /> --}}
                  </td>
                  <td>
                    <label class="switch">
                      <input class="subscriptionstatus" type="checkbox"  data-id="{{$cat->id}}" name="status" {{ $cat->is_subscription_enabled== '1' ? 'checked' : '' }}>
                      <span class="knob"></span>
                    </label>
                    {{-- <input  data-id="{{$cat->id}}" type="checkbox"  class="custom_toggle" name="status" {{ $cat->status == '1' ? 'checked' : '' }} /> --}}
                  </td>
                  ...
                  <td>
                    <label class="switch">
                      <input class="bundlestatus" type="checkbox"  data-id="{{$cat->id}}" name="status" {{ $cat->status == '1' ? 'checked' : '' }}>
                      <span class="knob"></span>
                    </label>
                    {{-- <input  data-id="{{$cat->id}}" type="checkbox"  class="custom_toggle" name="status" {{ $cat->status == '1' ? 'checked' : '' }} /> --}}
                  </td>
...
<script>
  ...
  $(function() {
    $('.subscriptionstatus').change(function() {
      ...
      var is_subscription_enabled = $(this).prop('checked') == true ? 1 : 0; 
      var id = $(this).data('id'); 
        $.ajax({
            type: "GET",
            dataType: "json",
            url: 'bundlecourse/subscription/status',
            data: {'is_subscription_enabled': is_subscription_enabled, 'id': id},
            success: function(data){
              console.log(id)
            }
        });
